...
from utils import yesno2bool, contact_me
...
class Editor(Tk):
    ...
    def _tocontinue(self):
        if not self.saved:
            ...
            if result == "yes":
                ...
                return False
        return True
    def _validate_lwp_data(self, data):
        ...
        if self._tocontinue():
            ...
            if lwp_file:
                try:
                    ...
                    if self._validate_lwp_data(lwp_data):
    ...
    def edit(self):
        ...
        if len(selection) == 0:
            ...
            showinfo("Information", "Select only ONE \"word-translation\" couple - you selected {}".format(len(selection)))
        else:
            edited = Edit("Edit", *self.wtree.item(selection[0])["values"])
            if hasattr(edited, "data"):
                self.wtree.delete(selection[0])
                self.wtree.insert("", END, values=edited.data)
                self.set_saved(False)
    def remove(self):
        if self.wtree.selection():
            if yesno2bool(show_msg("Warning", "All selected words' couples will be permanently deleted. Do you want to continue?\nNote this action cannot be undone!", "warning", "yesno")):
                self.wtree.delete(*self.wtree.selection())
                self.set_saved(False)
        else:
            showinfo("Information", "Choose something at first.\nIf you want to remove all the words from the list, click \"Clear\"")
    def clear(self):
        if self.wtree.get_children():
            if yesno2bool(show_msg("Warning", "All the couples from this list will be permanently deleted. Do you want to continue?\nNote this action cannot be undone!", "warning", "yesno")):
                self.wtree.delete(*self.wtree.get_children())
                self.set_saved(False)
        else:
...
class Edit(Toplevel):
    ...
    def ok(self, event=None):
        ...
        self.translation = self.translation_entry.get()
        self.data = (self.word, self.translation)
        self.destroy()
